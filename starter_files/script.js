...
class Factory {
  constructor(make = "Mazda", location = "USA", airbags = true, abs = true, warranty = "60,000 Miles / 3 Years") {
    ...
    this.airbags = location
    this.abs = abs
    this.warranty = warranty
  }
  massBuild(quantity, options) {
    // console.log("Building " + this.quantity + " " + this.options.color + " " + this.options.trim + " " + this.options.model + "'s." );
    console.log("Building " + quantity + " " + this.color + " " + this.trim + " " + this.model + "'s'" + options);
  }
...
// The following properties must be specified: model (mazda3), doors(4), color (red), enginetype (hybrid), transmission (automatic), trim (touring), wheels (base), audio (premium), seats (leather), and moonroof (true)
...
miataRf.customerBuild("Black", ["HID headlights", "sports suspension", "leather steering wheel", "heated seats", "adaptive cruise crontrol"] )
